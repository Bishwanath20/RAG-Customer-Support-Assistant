...
def run_evaluation(graph):
    """Run only the answerable queries through the pipeline, then score
    them with the RAG evaluation framework (precision/relevancy/faithfulness)."""
    from src.evaluation import evaluate_dataset, print_report
    ...
    print("\n🔬 Running RAG evaluation on answerable queries...")
    cases = []
    for q in eval_queries:
        state = run_query(graph, q)
        # Only evaluate queries that were actually answered (not escalated)
        if state["route"] == "answer":
            cases.append({
                "query"          : q,
                "answer"         : state["answer"],
                "retrieved_docs" : [d.page_content for d in state["retrieved_docs"]],
            })

    if not cases:
        print("  No answerable queries to evaluate.")
        return

    results = evaluate_dataset(cases)
...
def main():
    ...
    parser.add_argument("--eval",    action="store_true",
                        help="Run RAG evaluation (faithfulness, relevancy, precision)")
    parser.add_argument("--benchmark", action="store_true",
                        help="Run chunk-size ablation experiment")
    args = parser.parse_args()

    # Benchmark runs standalone — no API key or graph needed
    if args.benchmark:
